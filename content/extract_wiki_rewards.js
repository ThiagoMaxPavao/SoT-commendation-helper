...
function extractCommendationsFromWiki() {
  ...
  // Check existing storage before saving and showing popup
  chrome.storage.local.get(['wikiCommendationRewards'], (data) => {
    const prev = data.wikiCommendationRewards;
    const isNew = !prev;
    const isChanged = !isNew && !deepEqual(prev, result);
    ...
    if (isNew || isChanged) {
      ...
      // No change, do not update timestamp or show popup
      logger.log("No changes in rewards, not showing popup or updating timestamp.");
    }
  });
}
